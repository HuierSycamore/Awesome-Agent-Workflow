...
- `reference/change-impact-analysis-checklist.md`：用于设计完成后自检变更影响分析的完整性

同时加载密码算法敏感安全参数分析的参考文件（仅在功能涉及基础密码算法时使用）：
- `reference/sensitive-param-classification.md`：用于识别涉及算法的敏感参数类型和特殊要求
- `reference/sensitive-param-checklist.md`：用于设计完成后自检敏感参数分析的完整性
...
3. **分析这个答案，检查它是否触发了以下维度的新决策点（逐项过，不跳）**：
   ...
   - **密码算法敏感安全参数**：如果本次功能涉及基础密码算法（对称加密、AEAD、MAC、KDF、摘要、随机数、公钥算法、KEM、签名、HBS 等），是否需要在 DFX 章节填写"密码算法敏感安全参数分析"？涉及的算法类别、秘密参数、安全关键参数、派生敏感状态是否已识别？参考 `reference/sensitive-param-classification.md` 确认参数分类。
...
- **密码算法敏感安全参数分析**：如果本次功能涉及基础密码算法，DFX 章节 6.5 必须根据 `reference/sensitive-param-classification.md` 和 `reference/sensitive-param-checklist.md` 填写涉及算法与参数识别、参数安全约束、特殊安全资源和算法类别专项要求。如果功能不涉及密码算法，在 6.5 中标注"不涉及"。
- **软件变更影响分析**：DFX 章节 6.6 对所有功能设计均适用，必须填写变更分级与摘要、变更项清单、对外可观察变更分析、影响传导分析和兼容性矩阵。填写完成后使用 `reference/change-impact-analysis-checklist.md` 自检完整性。
